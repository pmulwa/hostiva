...
BEGIN
  ...
  -- Load platform fee settings (fall back to sensible defaults if missing).
  SELECT
  ...
  LOOP
    -- Skip fully-refunded cancellations. Only keep confirmed/completed,
    -- or cancellations where refund is 0% or exactly 50% of total.
    ...
    -- For 50%-refund cancellations, the host keeps 50% of the subtotal. No cleaning.
    IF v_booking.status = 'cancelled' AND v_refund_pct = 50 THEN
      v_subtotal := ROUND(v_subtotal * 0.5, 2);
      v_cleaning := 0;
    END IF;

    -- Compute fees the same way the platform does (see calculateFees in usePlatformSettings.ts):
    --   service fee total = subtotal × service_fee%
    --   service fee with tax = service fee × (1 + service_tax%)
    --   host service fee = full amount if charged to host, half if split, 0 if guest
    --   host commission = subtotal × host_commission%
    --   host commission tax = host commission × host_tax%
    --   net payout = subtotal − host service fee − (host commission + tax) + cleaning fee
