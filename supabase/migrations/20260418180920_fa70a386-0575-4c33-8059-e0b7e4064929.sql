-- 1) Update auto-post trigger function: use check_in_date as the posting date
...
BEGIN
  ...
  IF v_booking_ccy <> UPPER(COALESCE(v_base_currency, 'USD')) THEN
    INSERT INTO public.acct_external_bookings (
      ...
      'Hostly payout', NEW.check_in_date, 'AUTO:HOSTLY:' || NEW.id::text,
...
-- 2) Update bulk import function: use check_in_date as the posting date
  ...
  LOOP
    ...
    IF v_booking_ccy <> UPPER(COALESCE(v_base_currency, 'USD')) THEN
      INSERT INTO public.acct_external_bookings (
        ...
        v_gross, v_cleaning, v_commission, v_net,
        'Hostly payout', v_booking.check_in_date,
...
-- 3) Re-date EXISTING Hostly journal entries to use each booking's check-in date.
--    Match journal entries to their underlying booking via source_id (uuid stored as text).
UPDATE public.acct_journal_entries je
SET entry_date = b.check_in_date,
    updated_at = now()
FROM public.bookings b
WHERE je.source_type = 'booking'
  AND je.source_id IS NOT NULL
  AND je.source_id::uuid = b.id
  AND je.entry_date <> b.check_in_date;

-- 4) Also update payment_received_date on existing external Hostly rows to check-in date
UPDATE public.acct_external_bookings eb
SET payment_received_date = b.check_in_date,
    updated_at = now()
FROM public.bookings b
WHERE eb.notes LIKE 'AUTO:HOSTLY:' || b.id::text || '%'
  AND (eb.payment_received_date IS DISTINCT FROM b.check_in_date);
