-- Update import function to skip fully-refunded cancellations.
-- Include: confirmed, completed, and cancellations with 0% or 50% refund only.
...
BEGIN
  ...
  LOOP
    ...
    -- For 50%-refund cancellations, the host keeps 50% of total. Reduce gross proportionally.
    IF v_booking.status = 'cancelled' AND v_refund_pct = 50 THEN
      v_gross := ROUND(v_gross * 0.5, 2);
      v_cleaning := 0;
    END IF;
