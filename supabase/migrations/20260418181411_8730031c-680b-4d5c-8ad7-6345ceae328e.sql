-- 1) Add the setting column with a safe default
ALTER TABLE public.acct_settings
  ADD COLUMN IF NOT EXISTS entry_date_basis text NOT NULL DEFAULT 'check_in';

ALTER TABLE public.acct_settings
  DROP CONSTRAINT IF EXISTS acct_settings_entry_date_basis_check;

ALTER TABLE public.acct_settings
  ADD CONSTRAINT acct_settings_entry_date_basis_check
  CHECK (entry_date_basis IN ('check_in','check_out','booking_created'));

-- 2) Auto-post trigger: pick the entry date per host setting
...
BEGIN
  ...
  SELECT seeded, base_currency, COALESCE(entry_date_basis,'check_in')
    INTO v_settings_seeded, v_base_currency, v_basis
    FROM public.acct_settings WHERE host_id = NEW.host_id;
  IF v_settings_seeded IS NOT TRUE THEN RETURN NEW; END IF;

  v_entry_date := CASE v_basis
...
-- 3) Bulk import: same setting-driven entry date
...
BEGIN
  ...
  LOOP
    ...
    v_entry_date := CASE v_basis
      WHEN 'check_out' THEN v_booking.check_out_date
      WHEN 'booking_created' THEN v_booking.created_at::date
      ELSE v_booking.check_in_date
    END;
